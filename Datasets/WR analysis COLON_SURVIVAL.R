...
# Output folder.
# If your project folder exists, outputs are written there. Otherwise, outputs
# are written under the current working directory.
PROJECT_DIR <- "C:/Users/Lenovo/Desktop/Win ratio simulation"
if (!dir.exists(PROJECT_DIR)) PROJECT_DIR <- getwd()

OUTDIR <- file.path(PROJECT_DIR, "WR outputs colon")
if (!dir.exists(OUTDIR)) dir.create(OUTDIR, recursive = TRUE)

# Main real-data controls.
# These are set explicitly so the run is not affected by objects left over from
# earlier HCE1/DIG test runs in the R environment.
B_REAL <- 2000L
BATCH_SIZE_REAL <- 50L
MASTER_SEED <- 2026L
ALPHA <- 0.05
...
# For recurrence-first order: p weights recurrence; 1-p weights death.
...
# For the colon cancer trial, the first endpoint is death/survival time. A
# difference of only 1--3 months is usually too short for a clinically meaningful
# survival separation in an oncology follow-up setting, so the sensitivity grid
# starts at 6 months and then covers 1, 1.5, 2, 3, 4, and 5 years. These clinical
# candidates are still combined with first-endpoint data-informed candidates.
CLINICAL_T_MONTHS <- c(6, 12, 24, 36, 60)
MAX_T_GRID_SIZE <- 12
EPS_WR <- 1e-8
...
# Registry for the built-in survival::colon clinical-trial dataset.
# The colon dataset has two rows per subject: one for recurrence and one for
# death. We reshape it to the existing pipeline format:
#   Endpoint 1 (high priority): death / survival time
#   Endpoint 2 (lower priority): cancer recurrence
# Treatment comparison:
#   ARM = 1: Levamisole + 5-FU
#   ARM = 0: Observation
# The Levamisole-only arm is excluded to keep a clean two-arm comparison.

REAL_DATASETS <- data.frame(
  dataset_id = c("colon_lev5fu_vs_obs"),
  dataset_label = c("Colon"),

  # The custom loader below recognizes survival::colon and reshapes it into
  # the same ds$table.output + ds$hosp.times.list structure used by the rest of
  # the code. The core WR/permutation code is unchanged.
  source_type = c("package"),
  subject_file = c(""),
  recurrent_file = c(""),

  package_name = c("survival"),
  data_object = c("colon"),
  ...
  # Times in survival::colon are days; the custom colon reshaper converts them
  # to years before returning the standardized ds object.
  time_unit = c("days"),
  hosp_time_unit = c("days"),
  hosp_time_type = c("gap"),
...
ARM_TREATMENT_VALUES <- c("1", "treatment", "treated", "trt", "active", "intervention", "digoxin", "lev+5fu", "levamisole+5-fu")
...
load_package_data_object <- function(package.name, object.name, dataset.id = "realdata") {
  ...
  # First try the standard data() route. Some R installations do not expose
  # survival::colon through utils::data(), so we also add namespace/exported
  # object fallbacks below.
  ...
  if (!inherits(loaded, "error") && exists(object.name, envir = env, inherits = FALSE)) {
    return(get(object.name, envir = env, inherits = FALSE))
  }

  # Fallback 1: exported object, equivalent to package::object when available.
  exported.obj <- tryCatch(
    getExportedValue(package.name, object.name),
    error = function(e) NULL
  )
  if (!is.null(exported.obj)) {
    return(exported.obj)
  }

  # Fallback 2: namespace object, useful for lazy-loaded package datasets.
  ns <- tryCatch(asNamespace(package.name), error = function(e) NULL)
  if (!is.null(ns) && exists(object.name, envir = ns, inherits = FALSE)) {
    return(get(object.name, envir = ns, inherits = FALSE))
  }

  # Fallback 3: attached package environment, if library(package) already ran.
  pkg.env.name <- paste0("package:", package.name)
  if (pkg.env.name %in% search() && exists(object.name, envir = as.environment(pkg.env.name), inherits = FALSE)) {
    return(get(object.name, envir = as.environment(pkg.env.name), inherits = FALSE))
  }

  stop(
    "Could not load data object '", object.name, "' from package '", package.name,
    "' for ", dataset.id, ". Tried utils::data(), getExportedValue(), namespace lookup, and attached package lookup."
  )
}
...
# the original long format into:
#   table.output      one row per subject
#   hosp.times.list   lower-priority recurrence time list
# Internally, the old variable names NUMHOSP / HOSP_TIMES are retained because
# the WR engine expects those names; in the report they should be described as
# recurrence counts/times, not literal hospitalizations.
reshape.survival.colon.to.ds <- function(colon.raw,
                                         treatment.value = "Lev+5FU",
                                         control.value = "Obs",
                                         dataset.id = "colon_lev5fu_vs_obs") {
  colon.raw <- as.data.frame(colon.raw)

  required <- c("id", "rx", "time", "status", "etype")
  missing.required <- setdiff(required, names(colon.raw))
  if (length(missing.required) > 0) {
    stop("survival::colon is missing required columns: ",
         paste(missing.required, collapse = ", "))
  }

  to_numeric <- function(x) suppressWarnings(as.numeric(as.character(x)))

  colon.raw$rx_chr <- trimws(as.character(colon.raw$rx))
  colon.raw$etype_num <- to_numeric(colon.raw$etype)
  colon.raw$status_num <- to_numeric(colon.raw$status)
  colon.raw$time_days <- to_numeric(colon.raw$time)

  keep <- colon.raw$rx_chr %in% c(control.value, treatment.value)
  dat <- colon.raw[keep, , drop = FALSE]

  if (nrow(dat) == 0) {
    stop("No survival::colon rows remained after keeping rx in {",
  ...
  futime <- numeric(n)
  cnsr <- integer(n)
  recurrence.time.years <- rep(NA_real_, n)
  recurrence.status <- integer(n)
  rx.used <- character(n)
  ...
  for (ii in seq_along(ids)) {
    id.i <- ids[ii]
    d <- dat[dat$id == id.i, , drop = FALSE]

    rx.i <- unique(d$rx_chr)
    rx.i <- rx.i[!is.na(rx.i) & rx.i != ""]
    if (length(rx.i) == 0) stop("Missing rx for id = ", id.i)
    rx.used[ii] <- rx.i[1]

    arm[ii] <- ifelse(rx.used[ii] == treatment.value, 1L, 0L)

    death.row <- d[d$etype_num == 2, , drop = FALSE]
    recur.row <- d[d$etype_num == 1, , drop = FALSE]

    if (nrow(death.row) == 0) {
      # Very defensive fallback; normally colon has one death/censor row.
      death.time.days <- max(d$time_days, na.rm = TRUE)
      death.status <- 0L
    } else {
      death.row <- death.row[1, , drop = FALSE]
      death.time.days <- death.row$time_days[1]
      death.status <- as.integer(death.row$status_num[1] == 1)
    }

    futime[ii] <- death.time.days / 365.25
    cnsr[ii] <- death.status

    if (nrow(recur.row) > 0) {
      recur.row <- recur.row[1, , drop = FALSE]
      rec.time <- recur.row$time_days[1] / 365.25
      rec.status <- as.integer(recur.row$status_num[1] == 1)

      if (is.finite(rec.time) && rec.status == 1L && rec.time > 0 && rec.time <= futime[ii]) {
        recurrence.time.years[ii] <- rec.time
        recurrence.status[ii] <- 1L
        # The WR engine expects gap times. With at most one recurrence, the gap
        # is numerically the same as the absolute time from baseline.
        hosp.times.list[[ii]] <- rec.time
      } else {
        hosp.times.list[[ii]] <- numeric(0)
      }
    } else {
      hosp.times.list[[ii]] <- numeric(0)
    }
  ...
  table.output <- data.frame(
    ...
    SURVTIME = as.numeric(futime),
    CNSRTIME = as.numeric(futime),
    FREQHOSP = NA_real_,
    NUMHOSP = as.integer(lengths(hosp.times.list)),
    COLON_RX = rx.used,
    RECURRENCE_TIME = recurrence.time.years,
  ...
  list(
    ...
    import.note = data.frame(
      ...
      source_dataset = "survival::colon",
      comparison = paste(treatment.value, "vs", control.value),
      endpoint_1 = "death / survival time",
      endpoint_2 = "recurrence",
      n_subjects = nrow(table.output),
      n_control = sum(table.output$ARM == 0),
      n_treatment = sum(table.output$ARM == 1),
      deaths = sum(table.output$CNSR == 1, na.rm = TRUE),
      total_recurrences = sum(table.output$NUMHOSP, na.rm = TRUE),
      excluded_arm = "Levamisole only",
      time_unit = "converted from days to years",
      hosp_time_unit = "recurrence time converted from days to years",
      hosp_time_type = "gap list supplied; one recurrence maximum",
      stringsAsFactors = FALSE
    )
  )
}
...
load.real.dataset <- function(registry.row,
  ...
  if (source.type %in% c("package", "r_package", "data")) {
    ...
    # Special handling for survival::colon: the raw dataset is long-format
    # with one row for recurrence and one row for death per subject, so it must
    # be reshaped before it can enter the existing death + non-terminal-event
    # win-ratio pipeline.
    if (tolower(package.name) == "survival" && tolower(data.object) == "colon") {
      return(reshape.survival.colon.to.ds(subject.obj, dataset.id = dataset.id))
    }
  } else {
...
List fast_wr_core_revised_cpp(NumericVector futime,
  ...
  // Recurrence-first hierarchy counts.
  double H1_win = 0.0;
  double H1_loss = 0.0;
  double D2_win = 0.0;
  double D2_loss = 0.0;
...
run.logrank.tests <- function(ds) {
  ...
  data.frame(
    ...
    note = c(
      ...
      "Time to earliest of death or recurrence"
    ),
    stringsAsFactors = FALSE
  )
}
...
perm.test.revised <- function(ds,
  ...
  list(
    ...
    T.perm.tie.max.count = perm.tie.max.count,
    T.perm.tie.max.pr = perm.tie.max.pr,
    T.perm.tie.fixed.count = perm.tie.fixed.count,
    T.perm.tie.fixed.pr = perm.tie.fixed.pr,
...
tie_count_from_row <- function(x) {
  if (is.null(x) || length(x) == 0) return(NA_real_)
  if (is.data.frame(x) && nrow(x) == 0) return(NA_real_)
  if ("tie.count" %in% names(x)) return(as.numeric(x$tie.count[1]))
  if ("tie_count" %in% names(x)) return(as.numeric(x$tie_count[1]))
  if ("selected.tie.count" %in% names(x)) return(as.numeric(x$selected.tie.count[1]))
  return(NA_real_)
}

tie_pr_from_row <- function(x) {
  if (is.null(x) || length(x) == 0) return(NA_real_)
  if (is.data.frame(x) && nrow(x) == 0) return(NA_real_)
  if ("tie.pr" %in% names(x)) return(as.numeric(x$tie.pr[1]))
  if ("pr.tie" %in% names(x)) return(as.numeric(x$pr.tie[1]))
  if ("tie_proportion" %in% names(x)) return(as.numeric(x$tie_proportion[1]))
  if ("selected.tie.pr" %in% names(x)) return(as.numeric(x$selected.tie.pr[1]))
  return(NA_real_)
}

get_max_method_tie <- function(engine.out, method.id) {
  row <- switch(
    as.character(method.id),
    ordinaryWR = get_selected_weighted_row(engine.out, "death_first", 0.50),
    traditionalWR_hosp_first = get_selected_weighted_row(engine.out, "hospitalization_first", 0.50),
    traditionalOrderWR = get_selected_weighted_row(engine.out, engine.out$max.traditional.order$order[1], 0.50),
    maxWRp_primary = get_selected_weighted_row(engine.out, "death_first", engine.out$max.weighted.primary$p[1]),
    maxWRp_low = get_selected_weighted_row(engine.out, "death_first", engine.out$max.weighted.low$p[1]),
    maxWRp_full = get_selected_weighted_row(engine.out, "death_first", engine.out$max.weighted.full$p[1]),
    maxOrderWR_primary = get_selected_weighted_row(engine.out, engine.out$max.order.primary$order[1], engine.out$max.order.primary$p[1]),
    maxOrderWR_full = get_selected_weighted_row(engine.out, engine.out$max.order.full$order[1], engine.out$max.order.full$p[1]),
    maxWRt = engine.out$max.threshold,
    NULL
  )
  c(tie.count = tie_count_from_row(row), tie.pr = tie_pr_from_row(row))
}

get_fixed_method_tie <- function(engine.out, method.id, selected.fixed) {
  row <- switch(
    as.character(method.id),
    ordinaryWR = get_selected_weighted_row(engine.out, "death_first", 0.50),
    fixedTraditionalHospFirst = get_selected_weighted_row(engine.out, "hospitalization_first", 0.50),
    fixedTraditionalOrderWR = get_selected_weighted_row(engine.out, selected.fixed$traditional.order, 0.50),
    fixedWRp_primary = get_selected_weighted_row(engine.out, "death_first", selected.fixed$p.primary),
    fixedWRp_low = get_selected_weighted_row(engine.out, "death_first", selected.fixed$p.low),
    fixedWRp_full = get_selected_weighted_row(engine.out, "death_first", selected.fixed$p.full),
    fixedOrder_primary = get_selected_weighted_row(engine.out, selected.fixed$order.primary, selected.fixed$order.p.primary),
    fixedOrder_full = get_selected_weighted_row(engine.out, selected.fixed$order.full, selected.fixed$order.p.full),
    fixedWRt = get_selected_threshold_row(engine.out, selected.fixed$t),
    NULL
  )
  c(tie.count = tie_count_from_row(row), tie.pr = tie_pr_from_row(row))
}

matrix_col_mean <- function(x, col.name) {
  if (is.null(x) || is.null(col.name) || !(col.name %in% colnames(x))) return(NA_real_)
  mean(as.numeric(x[, col.name]), na.rm = TRUE)
}
...
make.selected.max.table <- function(test.out) {
  ...
  get_tie_count <- function(x) {
    if (is.null(x) || nrow(x) == 0) return(NA_real_)
    if ("tie.count" %in% names(x)) return(as.numeric(x$tie.count[1]))
    if ("tie_count" %in% names(x)) return(as.numeric(x$tie_count[1]))
    return(NA_real_)
  }
  get_tie_pr <- function(x) {
    if (is.null(x) || nrow(x) == 0) return(NA_real_)
    if ("tie.pr" %in% names(x)) return(as.numeric(x$tie.pr[1]))
    if ("pr.tie" %in% names(x)) return(as.numeric(x$pr.tie[1]))
    if ("tie_proportion" %in% names(x)) return(as.numeric(x$tie_proportion[1]))
...
clean.order.label <- function(x) {
  ...
  out <- gsub("hospitalization_first", "recurrence first", out)
  out <- gsub("_", " ", out)
  out
}
...
make.pathway.results.table <- function(test.out, logrank.results = NULL) {
  ...
  get_tie_count <- function(x) tie_count_from_row(x)
  get_tie_pr <- function(x) tie_pr_from_row(x)
...
threshold.candidate.source.table <- function(clinical.months = CLINICAL_T_MONTHS) {
  data.frame(
    ...
    candidate_type = "colon cancer clinical sensitivity candidates",
    how_used = paste(
      "These time-difference candidates are pre-specified sensitivity candidates for the death/survival endpoint.",
      "For the colon trial we use 6 months, 1 year, 2 years, 3 years, and 5 years because survival and recurrence differences are usually interpreted on a multi-month to multi-year oncology follow-up scale.",
      "They are combined with first-endpoint-only data-informed candidates; recurrence information is not used to select t."
    ),
    source_key = c(
      "survival::colon documentation; death-first win-ratio hierarchy; oncology follow-up time scale; RMST/time-difference interpretation"
    ),
    stringsAsFactors = FALSE
  )
}
...
choose.threshold.grid.primary <- function(ds,
  ...
  threshold.table <- data.frame(
    ...
    source_detail = ifelse(
      ...
      "Pre-specified colon-cancer sensitivity candidate on a multi-month/multi-year survival time scale.",
    ...
    reference_key = ifelse(
      ...
      "First-endpoint data only; no recurrence or composite endpoint information used"
    ),
    stringsAsFactors = FALSE
  )
...
#Fixed p = 0.50, compare death-first vs recurrence-first.
...
perm.test.revised <- function(ds,
  ...
  perm.tie.max.count <- matrix(NA_real_, nrow = B, ncol = length(max.names)); colnames(perm.tie.max.count) <- max.names
  perm.tie.max.pr <- matrix(NA_real_, nrow = B, ncol = length(max.names)); colnames(perm.tie.max.pr) <- max.names
  perm.tie.fixed.count <- matrix(NA_real_, nrow = B, ncol = length(fixed.names)); colnames(perm.tie.fixed.count) <- fixed.names
  perm.tie.fixed.pr <- matrix(NA_real_, nrow = B, ncol = length(fixed.names)); colnames(perm.tie.fixed.pr) <- fixed.names
  ...
  for (b in seq_len(B)) {
    ...
    for (nm in max.names) {
      tie.tmp <- get_max_method_tie(out.b, nm)
      perm.tie.max.count[b, nm] <- tie.tmp["tie.count"]
      perm.tie.max.pr[b, nm] <- tie.tmp["tie.pr"]
    }
    selected.fixed.for.tie <- list(
    ...
    for (nm in fixed.names) {
      tie.tmp <- get_fixed_method_tie(out.b, nm, selected.fixed.for.tie)
      perm.tie.fixed.count[b, nm] <- tie.tmp["tie.count"]
      perm.tie.fixed.pr[b, nm] <- tie.tmp["tie.pr"]
    }
  }
...
method.long.label <- function(method) {
  map <- c(
    ...
    traditionalWR_hosp_first = "Traditional WR, recurrence first",
    ...
    logrank_death = "Log-rank death",
    logrank_composite = "Log-rank composite",
    fixedTraditionalHospFirst = "Traditional fixed WR, recurrence first",
...
make.selected.max.table <- function(test.out) {
  ...
  data.frame(
    ...
    average.permutation.tie.count.max = sapply(method.ids, function(nm) matrix_col_mean(test.out$T.perm.tie.max.count, nm)),
    average.permutation.tie.pr.max = sapply(method.ids, function(nm) matrix_col_mean(test.out$T.perm.tie.max.pr, nm)),
    average.permutation.tie.count.fixed = sapply(fixed.ids, function(nm) matrix_col_mean(test.out$T.perm.tie.fixed.count, nm)),
    average.permutation.tie.pr.fixed = sapply(fixed.ids, function(nm) matrix_col_mean(test.out$T.perm.tie.fixed.pr, nm)),
    stringsAsFactors = FALSE
  )
}
...
make.pathway.results.table <- function(test.out, logrank.results = NULL) {
  ...
  out <- data.frame(
    ...
    average_permutation_tie_count_max = sapply(method.id, function(nm) matrix_col_mean(test.out$T.perm.tie.max.count, nm)),
    average_permutation_tie_pr_max = sapply(method.id, function(nm) matrix_col_mean(test.out$T.perm.tie.max.pr, nm)),
    average_permutation_tie_count_fixed = sapply(fixed.id, function(nm) matrix_col_mean(test.out$T.perm.tie.fixed.count, nm)),
    average_permutation_tie_pr_fixed = sapply(fixed.id, function(nm) matrix_col_mean(test.out$T.perm.tie.fixed.pr, nm)),
    B = test.out$B,
    stringsAsFactors = FALSE
  )
  if (!is.null(logrank.results)) {
    ...
    lr.rows <- data.frame(
      ...
      tie_count = NA_real_,
      tie_proportion = NA_real_,
      average_permutation_tie_count_max = NA_real_,
      average_permutation_tie_pr_max = NA_real_,
      average_permutation_tie_count_fixed = NA_real_,
      average_permutation_tie_pr_fixed = NA_real_,
      B = NA_integer_,
      stringsAsFactors = FALSE
    )
    out <- rbind_fill_base(out, lr.rows)
  }
  out
}
...
plot.order.curves <- function(engine.out, outdir, prefix) {
  ...
  abline(v = 0.5, lty = 3, lwd = 2)
  legend("topright",
         legend = c("death first", "recurrence first",
...
perm.test.revised.batch <- function(ds,
  ...
  for (bb in seq_len(n.batch)) {
    ...
    if (resume && file.exists(batch.file)) {
      ...
      batch.tie.max.count <- matrix(NA_real_, nrow = length(idx), ncol = length(max.names)); colnames(batch.tie.max.count) <- max.names
      batch.tie.max.pr <- matrix(NA_real_, nrow = length(idx), ncol = length(max.names)); colnames(batch.tie.max.pr) <- max.names
      batch.tie.fixed.count <- matrix(NA_real_, nrow = length(idx), ncol = length(fixed.names)); colnames(batch.tie.fixed.count) <- fixed.names
      batch.tie.fixed.pr <- matrix(NA_real_, nrow = length(idx), ncol = length(fixed.names)); colnames(batch.tie.fixed.pr) <- fixed.names
      ...
      for (r in seq_along(idx)) {
        ...
        for (nm in max.names) {
          tie.tmp <- get_max_method_tie(out.b, nm)
          batch.tie.max.count[r, nm] <- tie.tmp["tie.count"]
          batch.tie.max.pr[r, nm] <- tie.tmp["tie.pr"]
        }
        selected.fixed.for.tie <- list(
          traditional.order = fixed.traditional.order,
          p.primary = fixed.p.primary,
        ...
        for (nm in fixed.names) {
          tie.tmp <- get_fixed_method_tie(out.b, nm, selected.fixed.for.tie)
          batch.tie.fixed.count[r, nm] <- tie.tmp["tie.count"]
          batch.tie.fixed.pr[r, nm] <- tie.tmp["tie.pr"]
        }
      }
      batch.out <- list(
        max = batch.max,
        fixed = batch.fixed,
        tie.max.count = batch.tie.max.count,
        tie.max.pr = batch.tie.max.pr,
        tie.fixed.count = batch.tie.fixed.count,
        tie.fixed.pr = batch.tie.fixed.pr,
        death = batch.death,
        hosp = batch.hosp,
        threshold = batch.thr,
        selected = batch.selected
    ...
    if (!is.null(batch.out$tie.max.count)) perm.tie.max.count[idx, colnames(batch.out$tie.max.count)] <- batch.out$tie.max.count
    if (!is.null(batch.out$tie.max.pr)) perm.tie.max.pr[idx, colnames(batch.out$tie.max.pr)] <- batch.out$tie.max.pr
    if (!is.null(batch.out$tie.fixed.count)) perm.tie.fixed.count[idx, colnames(batch.out$tie.fixed.count)] <- batch.out$tie.fixed.count
    if (!is.null(batch.out$tie.fixed.pr)) perm.tie.fixed.pr[idx, colnames(batch.out$tie.fixed.pr)] <- batch.out$tie.fixed.pr
